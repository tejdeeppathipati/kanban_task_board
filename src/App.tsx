...
function App() {
  ...
  const [memberRoleDraft, setMemberRoleDraft] = useState<TeamRole>("member");
  ...
  const canEditWorkspace = activeWorkspace?.role !== "viewer";
  ...
  const loadWorkspaceAccess = useCallback(async (user: User) => {
    ...
    if (email) {
      const { error: claimError } = await supabase.rpc("claim_workspace_invites");
      if (claimError) {
  ...
  const createStarterData = useCallback(async (activeUserId: string) => {
    ...
    if (!existingMembers?.length) {
      await supabase.from("team_members").insert([
        { user_id: activeUserId, name: "You", role: "admin", color: MEMBER_COLORS[0] },
        { user_id: activeUserId, name: "Design Lead", role: "manager", color: MEMBER_COLORS[1] },
        { user_id: activeUserId, name: "Engineering", role: "member", color: MEMBER_COLORS[2] },
      ]);
    }
  ...
  async function changeWorkspace(nextWorkspaceId: string) {
    const nextWorkspace = workspaces.find((workspace) => workspace.ownerId === nextWorkspaceId);
    setWorkspaceId(nextWorkspaceId);
    setSelectedTaskId(null);
    setBoardScope(nextWorkspace?.role === "owner" ? "personal" : "team");
...
function TopBar({
  ...
  const displayName = isGuest ? "Guest workspace" : userName || userEmail.split("@")[0];
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-right">
        ...
        <button className="btn-new" type="button" onClick={onCreate} disabled={!canEdit}>
          <svg
            viewBox="0 0 12 12"
            fill="none"
            stroke="currentColor"
        ...
        <button
          ...
          title={isGuest ? "Sign in with email or Google" : "Sign out"}
        >
          ...
          <span className="account-copy">
            ...
            <small>{isGuest ? "Sign in" : "Sign out"}</small>
          </span>
        </button>
      </div>
    </header>
...
function BoardSummary({
  ...
  const canUsePersonalScope = activeWorkspace?.role === "owner";
  const isAll =
...
function WorkspaceTools({
  ...
  activeWorkspace: WorkspaceOption | null;
  memberDraft: string;
  memberRoleDraft: TeamRole;
  labelDraft: string;
  setMemberDraft: (value: string) => void;
  setMemberRoleDraft: (value: TeamRole) => void;
          ...
          <div className="collaborator-list">
            ...
            {collaborators.map((collaborator) => (
              <span className="collaborator-row" key={collaborator.id}>
                ...
                <span>{collaborator.role} · {collaborator.status}</span>
              </span>
            ))}
          </div>
          {isWorkspaceOwner && (
    ...
    >
      ...
      <div className="task-stack">
        {tasks.length ? (
          ...
          <button className="empty-column" type="button" onClick={onCreate} disabled={!canEdit}>
      ...
      <section className="timeline-section">
        ...
        <form className="comment-form" onSubmit={onAddComment}>
          <textarea
            ...
            disabled={!canEdit}
          />
          <button className="secondary-button" type="submit" disabled={saving || !canEdit}>
            Add comment
          </button>
        </form>
...
function TaskEditor({
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        event.preventDefault();
        if (canEdit && draft.title.trim()) onSubmit(draft);
      }}
    >
      <h2>{title}</h2>
      <label className="field">
      ...
      <div className="picker-block assignment-block">
        <div className="picker-heading">
          <span>Assign task</span>
          <small>{draft.assigneeIds.length || "No"} selected</small>
        </div>
        <div className="choice-list">
          {members.map((member) => (
            <label className="choice assignee-choice" key={member.id}>
              <input
                type="checkbox"
...
function getInitials(value: string) {
  ...
  const first = firstPart[0] ?? "U";
  const second = parts.length > 1 ? lastPart[0] : firstPart[firstPart.length - 1];
