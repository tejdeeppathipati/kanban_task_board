...
func (h *TeamMemberHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.WithRLS(r.Context(), userID, func(ctx context.Context, tx pgx.Tx) error {
		...
		for rows.Next() {
			...
			if err := rows.Scan(&m.ID, &m.UserID, &m.Name, &m.Color,
...
func (h *TeamMemberHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.WithRLS(r.Context(), userID, func(ctx context.Context, tx pgx.Tx) error {
		return tx.QueryRow(ctx, `
			INSERT INTO team_members (user_id, name, color)
			VALUES ($1, $2, $3)
			RETURNING id, user_id, name, color, avatar_url, created_at
		`, req.UserID, name, req.Color).Scan(
			&member.ID, &member.UserID, &member.Name, &member.Color,
